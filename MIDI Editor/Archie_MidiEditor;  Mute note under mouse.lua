--[[
   ...
   * Description: Mute note(s) under mouse
   * Author:      Archie
   * Version:     1.0
   * AboutScript: ---
   * О скрипте:   Отключить звук ноты под мышью
   * GIF:         ---
   ...
   *                  + initialе
    ...
    for i = 1, notecnt do;
        ...
        if startNote < ppqPosition and endNote > ppqPosition and noteRow == pitch then;
            if muted == false then;
                reaper.MIDI_SetNote(take,i-1,selected,true,startNote,endNote,chan,pitch,vel,false);
            end;
            break;
        end;
    end;
