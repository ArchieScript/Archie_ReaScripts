--[[
   ...
   *
   * Category:    MidiEditor
   * Description: Duplicate selected events to mouse cursor(snap relative)
   * Author:      Archie
   * Version:     1.0
   * AboutScript: ---
   * О скрипте:   Дублировать выбранные события(ноты) в курсор мыши(привязка относительно)
   * GIF:         ---
...
--]]
    ...
    for i = 1, notecnt do;
	   local _,selected,_,startppqpos,_,_,pitch,_ = reaper.MIDI_GetNote(take,i-1);
	   if selected == true then;
		  posFirstNotePpq = startppqpos;
		  shiftPitch = noteRow - pitch;
		  break 
	   end;
    end;
    ...
    for i = 1, reaper.MIDI_CountEvts(take) do;
	   ...
	   if sel == true then;
		  ...
		  table.insert(pitchT      ,      pitch + shiftPitch);
