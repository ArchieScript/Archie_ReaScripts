--[[
   * Category:    View 
   * Description: Layout Default on selected tracks TCP
   * Author:      Archie
   * Version:     1.01
   * AboutScript: Layout Default on selected tracks TCP
   * О скрипте:   Макет по умолчанию на выбранных дорожках TCP
   * GIF:         ---
...
--===========================================================================================]]
    ...
    for i = CountSelTrack-1,0,-1 do;
        ...
        local _,str = reaper.GetTrackStateChunk(SelTrack,"",true);
        ----
        local checks = select(2,str:match('(LAYOUTS) (".-")'));
        if checks then;
            if checks ~= '"'..layoutName..'"' then;
                local Layout = str:match('LAYOUTS ".-" ".-"');
                if not Layout then Layout = 'LAYOUTS "" ""' end;
                if Layout == 'LAYOUTS "" ""' then;
                    local Perf = str:match('PERF %d+');
    ...
    reaper.Undo_EndBlock("Layout Default on selected tracks TCP",-1);