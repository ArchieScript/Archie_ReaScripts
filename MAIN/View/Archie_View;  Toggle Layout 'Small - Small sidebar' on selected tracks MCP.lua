--[[
   * Category:    View 
   * Description: Toggle Layout 'Small - Small sidebar' on selected tracks MCP 
   * Author:      Archie
   * Version:     1.01
   * AboutScript: Toggle Layout 'Small - Small sidebar' on selected tracks MCP 
   * О скрипте:   Переключить макет «Малая - Малая боковая панель» на выбранных дорожках MCP
   * GIF:         ---
...
--===========================================================================================]]
    ...
    local layoutName,layoutName1,layoutName2 = _,"bb --- Small Sidebar","ba - Small";


    reaper.Undo_BeginBlock();


    local SelTrack,UndoName = reaper.GetSelectedTrack(0,0);
    ...
    if MCP ~= '"'..layoutName1..'"' then layoutName = layoutName1;
        UndoName = "Toggle Layout 'SMALL SIDEBAR' on selected tracks MCP";
    else layoutName = layoutName2;
        UndoName = "Toggle Layout 'SMALL' on selected tracks MCP";
    end;
    ---
