...
--]]
    ...
    local ENV_SEl_F_TR = false   --| true/false Выделить первый созданный трек автоматизации  / оставить выделения на предыдущем треке автоматизации
    ...
    for it = 1,CountSelIt do;
        ...
        if posItem < Point4-(1e-004) and posItem+lenItem > Point1+(1e-004) then;
            ...
            local sel;
            if Only_Sel_Track == true then;
                if #STrT == 0 then;
                    local LastTouchedTrack = reaper.GetLastTouchedTrack();
                    if Track == LastTouchedTrack then sel = 1 end;
                else;
                    for tb = 1,#STrT do;
                        if STrT[tb] == Track then sel = 1 break end;
                    end;
                end;
            else;
                sel = 1;
            ...
            if sel == 1 then;
                ...
                local Env = reaper.GetTrackEnvelopeByChunkName(Track,EnvChun);
    ...
    local Track = reaper.GetLastTouchedTrack() or reaper.GetTrack(0,0);
    reaper.SetOnlyTrackSelected(Track);
