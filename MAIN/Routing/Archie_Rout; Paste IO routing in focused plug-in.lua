--[[ 
   ...
   * 
   * Category:    Routing 
   * Description: Paste IO routing in focused plug-in 
   * Author:      Archie 
   * Version:     1.0 
   * Описание:    Вставить маршрутизацию ввода-вывода в сфокусированный плагин 
...
--]] 
    ...
    local function PasteOutput_FocusedFX_Pin(NameScript,clean,persist); 
        local retval,tracknumber,itemnumber,fxnumber = reaper.GetFocusedFX(); 
        local track = reaper.GetTrack(0,tracknumber-1); 
        if not track then return -1 end; 
    ...
    PasteInput_FocusedFX_Pin ("CopyIORoutingOfFocusedPlugIn",false,false); 
    PasteOutput_FocusedFX_Pin("CopyIORoutingOfFocusedPlugIn",false,false); 
