--[[
   ...
   *
   * Category:    Various
   * Description: Var;  Snap position and length of selected items to nearest grid.lua
   * Author:      Archie
   * Version:     1.0
...
--]]
    ...
    --=====================================================
    --http://forum.cockos.com/showthread.php?p=2272936#post2272936
    ...
    for i = 1,CountSelItem do;
        ...
        local Grid = Arc_GetClosestGridDivision(posIt);
        ---
        if posIt ~= Grid then;
            if not UNDO then;
                reaper.Undo_BeginBlock()reaper.PreventUIRefresh(1)UNDO=true;
            end;
            reaper.SetMediaItemInfo_Value(itemSel,"D_POSITION",Grid);
        end;
        ---
        local posIt = reaper.GetMediaItemInfo_Value(itemSel,"D_POSITION");
    ...
    if UNDO then;
        ...
        reaper.Undo_EndBlock("Snap position and length of selected items to nearest grid",-1);
        reaper.UpdateArrange();
    else;
        no_undo();
    end;
