...
--]]
    ...
    local function main(); 
        ...
        local function GetSelActionsActList();
            ...
            if reaper.APIExists('JS_Window_Find')then;
                selected_actions,sectionID,sectionName = GetSelectedActionsFromActionList();
                if selected_actions[1]then cmd  = selected_actions[1].cmd  end;
                if selected_actions[1]then name = selected_actions[1].name end;
            end;
            return (cmd or ''),(name or ''),selected_actions,sectionID,sectionName;
        end;
        -- v.1.20)-------------------------------------
        ...
        for val in string.gmatch(ExtState, "{&&(.-)&&}") do;
            ...
            if stop then;
                ...
                local tog = reaper.GetToggleCommandStateEx(secID,reaper.NamedCommandLookup(idT[#idT]));
                if tog == 1 then;
            ...
            if showMenu > 0 then;
                ...
                for val in string.gmatch(ExtState..'{&&=x=&&}',"{&&.-&&}") do;
                    ...
                    if x == showMenu then;
                        ...
                        ---
                        local id,act,tblAct,indexSec,Secname = GetSelActionsActList();
                        if secID == 0   and indexSec ~= 0 then id=''act=''tblAct={}end;
                        if secID==32060 and indexSec ~= 3 then id=''act=''tblAct={}end;
    ...
    if str then;
        ...
        if Section_csv == 0 then;
            newScript = 'Archie_Var; Popup menu single-level('..retvals_csv..').lua';
            local newPath = path..'/Popup menu single-level/Main';
            reaper.RecursiveCreateDirectory(newPath,0);
            newFile = newPath..'/'..newScript;
            str = '--'..newScript..'\n'..str;
        else;
            newScript = 'Archie_MidiEditor; Popup menu single-level('..retvals_csv..').lua';
            local newPath = path..'/Popup menu single-level/MidiEditor';
            reaper.RecursiveCreateDirectory(newPath,0);
            newFile = newPath..'/'..newScript;
            str = '--'..newScript..'\n'..str;
        end;
