...
--]]
    ...
    = nameT[#idT]:gsub('^[#|]*','%0!'); end; t[#t+1]="{&&"..idT[#idT]..'='..nameT2.."&&}"; end; stop=nil; b=b+1; if b>tonumber(string.char(49,48))then nameT[#idT]=string.char(35,36,32,32,32)..nameT[#idT]end; end; ExtState = table.concat(t); t = nil; local FDRT = 10 - #nameT; if FDRT <=0 then FDRT = 'demo'end;
    if ADD_UP_DOWN ~= 0 and ADD_UP_DOWN ~= 1 then ADD_UP_DOWN = 1 end; local LCK,LCK2; if #nameTRem==0 then LCK = '#'else LCK = ''end; if #nameTRem <2 then LCK2 = '#'else LCK2 = ''end; local showMenu,numbUpDown; local Full = 'Full version $'; Version = ' - v.'..(Version or '?.??'); local AddList = "> > > > "..(' '):rep(15)..FDRT.."|Add||"..LCK.."Remove|".."Remove All / Script||"..LCK.."Rename||"..LCK2.."Move||>• script|"..hdblock.."Hide Add Menu||# Ctrl+click - OPEN AGAIN - (New instance)||#"..section..' '..Version..'||'..Full..'|<|<|';
    ...
    "Eng:\nScript with this name already exists.\nUpdate script by saving the list - Yes\nUpdate script Without saving the list - No\nNot update script - Cancel"; MB_W = reaper.MB(strMb,'Update popup list - '..retvals_csv,3); if MB_W == 2 then;no_undo()return;end; end; end; local newScriptX; if Section_csv
    == 0 then; newScriptX = 'Archie_Var;'..(' '):rep(2)..'Popup menu single-level('..retvals_csv..').lua'; else; newScriptX = 'Archie_MidiEditor;'..(' '):rep(2)..'Popup menu single-level('..retvals_csv..').lua'; end; local FileX = path..'/'..newScriptX; local fileX = io.open(FileX,'r'); if fileX then; fileX:close();
