--[[
   ...
   * Description: Nudge right edge left to nearest grid
   * Author:      Archie
   * Version:     1.0
   * AboutScript: Nudge right edge left to nearest grid
   * О скрипте:   Подтолкнуть правый край влево к ближайшей сетке
   * GIF:         ---
...
--===========================================================================================]]
    ...
    if ToggleGridLines == 1 then;
        ...
        local Grid = reaper.BR_GetPrevGridDivision(endLoop);
        reaper.GetSet_LoopTimeRange(1,0,startLoop,Grid,0);

        reaper.Undo_EndBlock("Nudge right edge left to nearest grid",-1);
