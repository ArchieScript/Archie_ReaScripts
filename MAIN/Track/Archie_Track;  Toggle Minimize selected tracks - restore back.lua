--[[
   ...
   *
   * Category:    Track
   * Description: Toggle Minimize selected tracks - recover back
   * Author:      Archie
   * Version:     1.01
   * AboutScript: ---
   * О скрипте:   Переключение Минимизировать выбранные треки - восстановить назад
   * GIF:         ---
...
--]]
    ...
    if Height > 30 then;
        ...
        for i = 1,reaper.CountSelectedTracks(0) do;
            ...
            local height = GetHeightTrack(SelTrack);
            if height > 30 then;
                local GUID = reaper.GetTrackGUID(SelTrack);
                reaper.SetProjExtState(0,extname,GUID,height);
                reaper.SetMediaTrackInfo_Value(SelTrack,"I_HEIGHTOVERRIDE",30);
            end;
        end;
    else;
        ...
        for i = 1,reaper.CountSelectedTracks(0) do;
            ...
            if val then;
                ...
                reaper.SetMediaTrackInfo_Value(SelTrack,"I_HEIGHTOVERRIDE",0);
            end; 
        end;
    end;
