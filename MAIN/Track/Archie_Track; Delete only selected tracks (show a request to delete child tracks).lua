...
--============================================================]]
    ...
    local Save,Folders_present,query, GUIDPre = {};
    for i = 1, CountSelTrack do;
        local selTrack = reaper.GetSelectedTrack(0,i-1);
        local fold = reaper.GetMediaTrackInfo_Value(selTrack, "I_FOLDERDEPTH");
        if fold == 1 then Folders_present = 1 break end;
    end;



    if Folders_present then;
        query = reaper.ShowMessageBox( "Delete children from selected folders ?\n"..
                                             "Удалить дочерние треки у выбранных папок ?",
                                             "Delete only selected tracks (show a request to del...",3);
        if query == 2 then no_undo() return end;
    end;
            ...
            elseif Depth > 0 then;
                ...
                for i = 1,reaper.CountTracks(0) do;
                    ...
                    if GUID == GUIDPre then;
                        ...
                        local tr = reaper.GetTrack(0,num-1);--ins
                        reaper.DeleteTrack(tr);--ins
                        break
                    end;
                end;
                RestoreSelTracksGuid(Save);
    ...
    reaper.Undo_EndBlock("Delete only selected tracks (show a request to delete child tracks)",-1);
    reaper.PreventUIRefresh(-1);