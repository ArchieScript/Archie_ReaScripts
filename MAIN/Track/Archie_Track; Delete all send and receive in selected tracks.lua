--[[
   * Category:    Item
   * Description: Delete all send and receive in selected tracks
   * Author:      Archie
   * Version:     1.02
   * AboutScript: ---
   * О скрипте:   ---
   ...
   --======================================================================================
    ...
    for i = 1, CountSelTracks do;
        ...
        if not Undo then;
            ...
            local CountReceives = reaper.GetTrackNumSends(Seltrack,-1);
            if CountSend > 0 or CountReceives > 0 then;
                reaper.Undo_BeginBlock();
                Undo = true;
            end;
        end;
        Arc.RemoveAllSendTr(Seltrack,0);
        Arc.RemoveAllSendTr(Seltrack,-1);
    end;
    reaper.PreventUIRefresh(-1);

    if Undo then;
        reaper.Undo_EndBlock("Delete all send and receive in selected tracks",-1);
    else;
        Arc.no_undo();
    end;