--[[
   * Category:    Track
   * Description: Volume reset on selected tracks (0 db)
   * Author:      Archie
   * Version:     1.0
   * AboutScript: Volume reset on selected tracks (0 db)
   * О скрипте:   Сброс громкости на выбранных треках (0 дБ)
   * GIF:         ---
   ...
   *              +  initialе / v.1.0 [02042019]
   ...
   ========================================================================================]]
    ...
    local CountSelTrack,Undo = reaper.CountSelectedTracks(0);
    if CountSelTrack == 0 then no_undo() return end;


    for i = 1, CountSelTrack do;
	   local SelTrack = reaper.GetSelectedTrack(0, i-1);
	   local vol = reaper.GetMediaTrackInfo_Value(SelTrack,"D_VOL")== 1;
	   if not vol then;
    ...
    if Undo then;
	   reaper.Undo_EndBlock("Volume reset on selected tracks (0 db)",-1);
    else;
	   no_undo();
    end;
