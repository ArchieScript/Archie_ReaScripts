--[[
   ...
   *
   * Category:    Track
   * Description: Track; Show selected tracks in TCP and MCP.lua
   * Author:      Archie
   * Version:     1.02
...
--]]
  ...
  --======================================================================================
    ...
    for i = 1,CountSelTracks do;
        ...
        local VisibM = reaper.IsTrackVisible(trackSel,true);
        local VisibT = reaper.IsTrackVisible(trackSel,false);
        if not VisibM or not VisibT then;
            if not UNDO then;
            ...
            if not VisibM then;
                reaper.SetMediaTrackInfo_Value(trackSel,'B_SHOWINMIXER',1);
            end;
            if not VisibT then;
    ...
    if UNDO then;
        ...
        reaper.PreventUIRefresh(-1);
        reaper.Undo_EndBlock('Show selected tracks in TCP and MCP',-1);
    else;
        no_undo();
    end;
