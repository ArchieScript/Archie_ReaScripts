--[[
   ...
   *
   * Category:    Track
   * Description: Track; Remove prefix name of selected track.lua
   * Author:      Archie
   * Version:     1.02
...
--]]
    ...
    local NAME_PREFIX = 'HIDE_'; -- Имя удаляемого префикса
    ...
    if USER_INPUTS then;
        local title = 'Remove prefix name of selected track';
    ...
    for i = CountSelTrack-1,0,-1 do;
        ...
        if No_REGIST then;
            name2 = name:sub(-#((name:upper()):gsub('^'..(NAME_PREFIX:upper()),'')));
        else;
            name2 = name:gsub('^'..NAME_PREFIX,'');
        end;
        if name ~= name2 then;
            if not UNDO then;
            ...
            reaper.GetSetMediaTrackInfo_String(SelTrack,"P_NAME",name2,1);
        end;
    end;
    ...
    if UNDO then;
        reaper.PreventUIRefresh(-1);
        reaper.Undo_EndBlock('Remove prefix name '..NAME_PREFIX,-1);
    else;
        no_undo();
    end;
