--[[
   * Category:    Fx
   * Description: Rename last selected FX in chain to selected tracks in preset name
   * Author:      Archie
   * Version:     1.0
   * AboutScript: Rename last selected FX in chain to selected tracks in preset name
   * О скрипте:   Переименовать последний выбранный FX в цепочке в выбранных дорожках на имя пресета
   * GIF:         ---
   ...
   * Changelog:   +  initialе / v.1.0 [260219]
   ...
   --======================================================================================
    ...
    if undo then;
        reaper.Undo_EndBlock("Rename last selected FX in chain to selected tracks in preset name",-1);
    else;
        Arc.no_undo();
    end;