--[[
   * Category:    Item
   * Description: Move all notes in all midi items in semitones(User Input)
   * Author:      Archie
   * Version:     1.0
   * AboutScript: Move all notes in all midi items in semitones(User Input)
   * О скрипте:   Перемещение всех нот во всех миди элементах в полутонах (пользовательский ввод)
   * GIF:         ---
   ...
   * Changelog:   + initialе / v.1.0[24122018]
--============================================================================================|
...
--===========================================================================================]]
    ...
    if Midi ~= "Active" then reaper.MB("No Midi items\n\nНет миди элементов","Error",0)no_undo()return end;


    local ret, val = reaper.GetUserInputs("Move all notes in all midi items in sem...",1,
                                  "________| Set value: |_______>,extrawidth=10", 0);
    val = tonumber(val);
    if not ret or val == 0 or not val then no_undo() return end;
    ...
    if Undo_BegBlock == "Active" then;
        ...
        reaper.Undo_EndBlock("Move all notes in all midi items in semitones(User Input)",-1);
    else;
        no_undo();
    end;