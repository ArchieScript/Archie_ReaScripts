--[[
   ...
   * О скрипте:   Разделить выбранный элемент(ы) под курсором мыши и все выбранные
                      элементы в этой позиции или элемент под курсором мыши (выберите справа)
...
--=============================================================================================]]
    ...
    local function Split_item_sel_right(item,Pos,Val_Fade_In_Out, Length_Item,Unselected_rest);
        ...
        if pos < Pos and len + pos >= Pos then;
            ...
            if Split then;
                ...
                reaper.SetMediaItemInfo_Value(Split, "B_UISEL",1);
    ...
    local TrackNumber,Split,item = Split_item_sel_right(item,Pos,Val_Fade_In_Out, Length_Item,Unselect_All_Rest_Items);
                    ...
                    for i2 = 1,CountTrItems do;
                        ...
                        if pos <= Pos and len + pos >= Pos then;
                            if reaper.GetMediaItemInfo_Value( item, "B_UISEL")== 1 then;
                                Split_item_sel_right(item,Pos,Val_Fade_In_Out, Length_Item,Unselect_All_Rest_Items);
                                break;
                            end;
                        else;
                            if Unselect_All_Rest_Items == 1 then;
