--[[
   ...
   *
   * Category:    Item
   * Description: Move selected items up by one visible track(skip minimized track)
   * Author:      Archie
   * Version:     1.0
   * Описание:    Переместить выбранные элементы вверх на одну видимую дорожку (пропустить свернутую дорожку)
...
--]]
    ...
    if Undo then;
        reaper.Undo_EndBlock("Move selected items up by "..HowMuchUp.." visible track(skip minimized track)",-1);
    else;
        no_undo();
    end;
