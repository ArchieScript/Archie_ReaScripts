--[[
   ...
   * Changelog:
   *              v.1.02 [09.11.19]
   *                  +  Fixed bug cleared check box (Move edit cursor when pasting/inserting media)
...
--]]
    ...
    local itClkMvCr = reaper.SNM_GetIntConfigVar("itemclickmovecurs",0);
    if itClkMvCr&8 ~= 0 then;
        reaper.SNM_SetIntConfigVar("itemclickmovecurs",itClkMvCr&~(itClkMvCr&8));
    end;
    ...
    if #SelItemT == 0 then goto END end;
    do;
    ----
        ...
        if moveEnv == 0 then;
            ...
            for i = #SelItemT,1,-1 do;
                ...
                if not len2 or len == len2 then;
                    ...
                    reaper.SetMediaItemInfo_Value(SelItemT[i],"D_LENGTH",len/3);
                    len2 = len;
            ...
            for i = 1,reaper.CountSelectedTracks(0) do;
                ...
                for ii = 1,reaper.CountTrackMediaItems(track) do;
                    ...
                    if reaper.GetMediaItemInfo_Value(item,"B_UISEL")==1 then;
                        ...
                        if pos+len < firstEndIt then firstEndIt = pos+len SecEndItem = pos+len+len end;
                    end
                end;
            end;
            ----
            ...
            reaper.SetEditCurPos(SecEndItem,false,false);
            reaper.Main_OnCommand(40058,0)--P
            for i = 1, reaper.CountSelectedMediaItems(0) do;
                table.insert(SelItemT_X,reaper.GetSelectedMediaItem(0,i-1));
            end;
            ...
            for i = 1, #SelItemT do;
                ...
                reaper.SetMediaItemInfo_Value(SelItemT[i],"D_LENGTH",len/3);
                reaper.SetMediaItemInfo_Value(SelItemT[i],"B_UISEL",1);
                reaper.SetEditCurPos(pos+len/3,false,false);
                reaper.Main_OnCommand(40698,0);--C
                reaper.Main_OnCommand(40058,0);--P
    ...
    ::END::
    if itClkMvCr&8 ~= 0 then;
       reaper.SNM_SetIntConfigVar("itemclickmovecurs",itClkMvCr);
    end;
