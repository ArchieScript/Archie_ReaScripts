--[[
   ...
   *
   * Category:    Item
   * Description: Item; Apply track-take FX to items as new take(lock act take)Offline all fx track.lua
   * Author:      Archie
   * Version:     1.0
...
--]]
    ...
    for i = 1,#t2 do;
        ...
        for i2 = 1,FXCount do;
            reaper.TrackFX_SetOffline(t2[i],i2-1,true);
        end;
    end;
    ----
    ...
    reaper.Undo_EndBlock('Apply track/take FX to items(lock active take)Offline all fx track',-1);
    reaper.PreventUIRefresh(-1);
    if ConfigUndo then reaper.SNM_SetIntConfigVar('undomask',ConfigUndo)end;
