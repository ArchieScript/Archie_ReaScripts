--[[
   * Category:    Item
   * Description: Switch item source file to next in folder restore original size(only Video)
   * Author:      Archie
   * Version:     1.0
   * AboutScript: Switch item source file to next in folder restore original size(only Video)
   * О скрипте:   ереключить исходный файл элемента на следующий в
...
--===========================================================================================]]
    ...
    if Undo == "Active" then;
        reaper.Undo_EndBlock("Switch item source file to next in folder restore original size(only Video)",-1);
    else;
        Arc.no_undo();
    end;
    ----
