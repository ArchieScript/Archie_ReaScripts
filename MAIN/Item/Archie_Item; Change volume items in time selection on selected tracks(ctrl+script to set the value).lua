--[[
   ...
   *                  +  initialе
   ...
   =======================================================================================]]
    ...
    if startLoop == endLoop then Arc.no_undo() return end;
    ...
    for i = 1,CountSelTrack do;
        ...
        for i2 = CountTrItem-1,0,-1 do;
            ...
            if not reaper.TakeIsMIDI(take)then;
                if pos < endLoop and pos+len > endLoop then;
                    reaper.SplitMediaItem(item,endLoop);
                end;
                --
                if pos < startLoop and pos+len > startLoop then;
                    reaper.SplitMediaItem(item,startLoop);
                end;
            end;
        end;
        ---------------------
        ...
        for i2 = CountTrItem-1,0,-1 do;
            ...
            local pos = reaper.GetMediaItemInfo_Value(item, "D_POSITION");
            local len = reaper.GetMediaItemInfo_Value(item, "D_LENGTH");
            if pos >= startLoop and pos+len <= endLoop then;
                local take = reaper.GetMediaItemTake(item,0);
                if not reaper.TakeIsMIDI(take)then;
