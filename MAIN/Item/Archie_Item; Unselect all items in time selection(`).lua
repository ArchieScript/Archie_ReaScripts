--[[
   * Category:    Item
   * Description: Unselect all items in time selection
   * Author:      Archie
   * Version:     1.0
   * AboutScript: Unselect all items in time selection
   * О скрипте:   Отменить выбор всех элементов в выборе времени
   * GIF:         ---
   ...
   *              +  initialе / v.1.0 [01042019]
   ...
   ========================================================================================]]
    ...
    for i = reaper.CountSelectedMediaItems(0)-1,0,-1 do;
        local SelItem = reaper.GetSelectedMediaItem(0,i);
        local PosIt = reaper.GetMediaItemInfo_Value(SelItem,"D_POSITION");
        if PosIt >= StartTimeSel and PosIt < EndTimeSel then;
    ...
    if Undo then;
        reaper.Undo_EndBlock("Unselect all items in time selection",-1);
    else;
        no_undo();
    end;
