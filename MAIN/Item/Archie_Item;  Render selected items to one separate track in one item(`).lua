--[[
   * Category:    Item
   * Description: Render selected items to one separate track in one item
   * Author:      Archie
   * Version:     1.0
   * AboutScript: Render selected items to one separate track in one item
   *              Analog Studio one "Mixdown selection", only understands time selection
   * О скрипте:   Рендеринг выбранных элементов на одну отдельную дорожку в один файл
   *              Аналог Студия один "Mixdown selection", только понимает выбор времени
   ...
   * Changelog:   +  initialе / v.1.0 [210219]
   ...
   --======================================================================================
    ...
    reaper.PreventUIRefresh(-1);
    reaper.Undo_EndBlock("Render selected items to one separate track in one item",-1);
