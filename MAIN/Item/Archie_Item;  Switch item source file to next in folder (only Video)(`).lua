--[[
   * Category:    Item
   * Description: Switch item source file to next in folder (only Video))
   * Author:      Archie
   * Version:     1.03
   * AboutScript: Switch item source file to next in folder (only Video))
   * О скрипте:   Переключить исходный файл элемента на следующий в папке (только видео)
...
--===========================================================================================]]
    ...
    if Undo == "Active" then;
        reaper.Undo_EndBlock("Switch item source file to next in folder (only Video))",-1);
    else;
        Arc.no_undo();
    end;
    ----
