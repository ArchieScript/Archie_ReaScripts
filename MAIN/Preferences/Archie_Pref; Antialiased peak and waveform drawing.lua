--[[               
   ...
   * Description: Antialiased peak and waveform drawing 
   * Author:      Archie 
   * Version:     1.0 
   * AboutScript: Preferences -> Peaks/Waveforms -> Name Script 
   * О скрипте:   Сглаженный пик и волновой рисунок 
...
--]]  
    ...
    -- / Antialiased peak and waveform drawing / -- 
    if ConfigVar&1024 == 1024 then; 
        SetConfigVar = ConfigVar &~ (ConfigVar&1024); 
    else; 
        SetConfigVar = ConfigVar | 1024; 
    end; 
    ...
    local function loop(); 
        ...
        local ConfigVar = reaper.SNM_GetIntConfigVar("peaksedges",0)&1024; 
         
        if ConfigVar == 1024 then; 
