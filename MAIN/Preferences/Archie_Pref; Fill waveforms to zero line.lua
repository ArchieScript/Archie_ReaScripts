--[[               
   ...
   * Description: Fill waveforms to zero line 
   * Author:      Archie 
   * Version:     1.0 
   * AboutScript: Preferences -> Peaks/Waveforms -> Name Script 
   * О скрипте:   Заполнение сигналов до нулевой линии 
...
--]]  
    ...
    -- / Fill waveforms to zero line / -- 
    if ConfigVar&2048 == 2048 then; 
        SetConfigVar = ConfigVar &~ (ConfigVar&2048); 
    else; 
        SetConfigVar = ConfigVar | 2048; 
    end; 
    ...
    local function loop(); 
        ...
        local ConfigVar = reaper.SNM_GetIntConfigVar("peaksedges",0)&2048; 
         
        if ConfigVar == 2048 then; 
